...
const Hero = () => {
  ...
  const heroImage = branding?.heroBanner || hero_1;
  const title = branding?.heroTitle || 'Modern Couture';
  const subtitle = branding?.heroSubtitle || 'Luxury Essentials';
  const buttonText = branding?.heroButtonText || 'Explore Now';

  return (
    <div className='relative w-full h-[600px] sm:h-[750px] overflow-hidden rounded-[2.5rem] sm:rounded-[3.5rem] bg-[#1a1a1a] mb-12 lg:mb-20 shadow-2xl group'>
      
      {/* ── Background Color Split ── */}
      <div className='absolute inset-0 flex flex-col md:flex-row'>
        <div className='w-full md:w-[45%] h-full bg-[#1a1a1a] flex flex-col justify-center px-12 md:px-20 z-10'>
           {/* Text Content */}
           <div className='space-y-8 animate-reveal'>
              ...
              <h1 className='text-6xl md:text-8xl font-black text-white uppercase tracking-tighter leading-[0.9] whitespace-pre-line'>
                {title}
              </h1>

              <p className='text-gray-400 text-xs md:text-sm font-medium uppercase tracking-widest leading-loose max-w-sm'>
                {subtitle}
              </p>

              <div className='pt-4'>
                <button onClick={() => navigate('/collection')} className='px-12 py-5 bg-[#D2B48C] text-black font-black text-[10px] uppercase tracking-[0.3em] rounded-full hover:bg-white transition-all duration-300 shadow-xl hover:scale-105 active:scale-95'>
                  {buttonText}
                </button>
              </div>
           </div>
